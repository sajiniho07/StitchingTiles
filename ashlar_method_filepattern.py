    ...
    try:
        ...
        for i in range(0, width, piece_width):
            for j in range(0, height, piece_height):
                ...
                piece_name = f"{base_filename}_X{i//piece_width + j//piece_height + 1}_Y{index}.tif"
...
def merge_tiff_files(files_dir, output_dir, tiff_files, overlap=0.10):
    ...
    tiff_command = [
        ...
        "-o",
        tiff_output,
        f"filepattern|{files_dir}" +  "|pattern=img4_X{row:1}_Y{col:1}.tif" + f"|overlap={overlap}|pixel_size=0.377454",
    ]
...
merge_tiff_files(temp_dir, output_dir, tiff_files, overlap=0.15)
